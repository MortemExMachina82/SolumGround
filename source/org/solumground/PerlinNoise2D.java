...
public class PerlinNoise2D {
    ...
    private static byte [] noisemap = new byte[sizeX*sizeY];
    public static byte [] getNoiseMap(){
        time += 0.01;
        for(int y = 0; y < sizeX; y++){
            for(int x = 0; x < sizeY; x++){
                double dx = (double) x / sizeX;
                double dy = (double) y / sizeY;
                int frequency = 6;
                double noise = noise((dx * frequency) + time, (dy * frequency) + time);
                noise = (noise - 1) / 2;

                byte finalValue = (byte)(noise*255);
                noisemap[y*sizeX + x] = finalValue;
            }
        }
        return noisemap;
    }
